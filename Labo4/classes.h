...
enum modes {NOMODE,INT_FLOAT,INT_INT_INT}; // nustras nuevas constantes (parametros)
enum parameters {ELEMENT_LENGTH,const_tau,const_k,const_lambda,const_nu,const_psi,const_alpha,const_delta,const_eta};
...
class mesh{
    ...
    public:
        void setParameters(float l,float tau,float k,float lambda,float nu, float psi,float alpha, float delta,float eta){
            parameters[ELEMENT_LENGTH]=l;
            parameters[const_tau]=tau;
            parameters[const_k]=k;
            parameters[const_lambda]=lambda;
            parameters[const_nu]=nu;
